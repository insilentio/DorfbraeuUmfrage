---
title: "Umfrage"
format: html
execute: 
  warning: false
  message: false
toc: true
---

# Umfrage zur ersten Serie unserer Biere

## Fragebogen

Bei der ersten Serie unserer Sude haben wir bei jedem Kistli jeweils einen Fragebogen mitgegeben:

![](images/degu.png){fig-align="center" width="564"}

## Rückmeldungen

Die Rückmeldungen siehen folgendermassen aus:

```{r}
#| label: csv
#| echo: false
# read the csv

library(tidyverse, warn.conflicts = FALSE, quietly = TRUE)
library(here)
options(knitr.kable.NA = '')

tabelle <- read_csv(here("Data/tabelle.csv")) |> 
  filter(!if_all(c(3:6), ~ is.na(.)))

knitr::kable(tabelle,
             align = c("l", "l", "c", "c", "c", "c", "l"))
```

## Resultate

Zusammengefasst präsentiert sich das folgendermassen:

```{r}
#| label: pivot
#| echo: false
options(knitr.kable.NA = '')

pivot <- tabelle |> 
  group_by(Biersorte) |> 
  summarise(across(c(2,3,4), \(x) mean(x, na.rm = TRUE)),
            across(c(5), \(x) sum(x, na.rm = TRUE)/n()))

knitr::kable(pivot, digits = c(0, 1, 1, 1, 2))
```

## Grafik

Grafisch sieht das so aus:
```{r}
#| echo: false
#| output: false

# make the data tidy
pivot <- pivot |> 
  ungroup() |> 
  pivot_longer(-c(1),
               names_to = "Kategorie", values_to = "Wert") |> 
  mutate(Biersorte = factor(Biersorte,
                            levels = c("Sonnenberg", "Oberdorf", "Oeggisbüel", "Isisbüel", "Ludretikon"),
                            ordered = TRUE)) |> 
  mutate(Wert = round(Wert, 2))

# create theme
mytheme <- theme(axis.title.x = element_blank(),
                 legend.position = "bottom",
                 panel.grid.major.x = element_blank(),
                 text = element_text(colour = "darkgrey"),
                 plot.title = element_text(colour = "darkgrey", size = 11, hjust = 0.5),
                 plot.background = element_rect(colour = "darkgrey", fill=NA, linewidth =.5) )

# define colors
mycolor <- c("#E7E04A", "#AA4DA5", "#6DBE78", "#E37237", "#DA3832")
```


### Bewertung:

```{r}
#| label: plot_values
#| echo: false

pivot |>
  filter(Kategorie != "Favorit") |> 
  ggplot(aes(y = Wert, x = Kategorie, fill = Biersorte)) +
    geom_col(position = "dodge") +
    geom_text(aes(label = Wert, y = Wert + 0.05), colour = "darkgrey", position = position_dodge(0.9), vjust = 0) +
    expand_limits(y = c(0,5)) +
    scale_fill_manual(values = mycolor) +
    mytheme
  
```

### Favoriten:

```{r}
#| label: plot_favorite
#| echo: false

pivot |> 
  filter(Kategorie == "Favorit") |> 
  ggplot(aes(y = Wert, x = Kategorie, fill = Biersorte)) +
    geom_col(position = "dodge") +
    geom_text(aes(label = paste(Wert*100, "%"), y = Wert + 0.05), colour = "darkgrey", position = position_dodge(0.9), vjust = 0) +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = mycolor) +
  mytheme
  
```